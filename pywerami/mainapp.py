...
class PyWeramiWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, filename=None, parent=None):
        ...
        ).clicked.connect(self.restore_props)
    ...
    def import_data(self, filename=None):
        ...
        if filename:
            if filename.lower().endswith(".tab"):
                self.data = GridData.from_tab(
                    filename, degrees=self.settings.value("degrees", False, type=bool)
                )
            elif filename.lower().endswith(".tci"):
                self.data = GridData.from_tci(
                    filename, degrees=self.settings.value("degrees", False, type=bool)
                )
            else:
    ...
    def openProject(self, checked, projfile=None):
        ...
        if projfile is None:
            ...
            projfile = qd.getOpenFileName(
                self, "Open project", os.path.expanduser("~"), filt
            )[0]
    ...
    def step_from_levels(self):
        if self.ready:
            ...
            if self.setlevels.isChecked():
                step = (float(self.levelmax.text()) - float(self.levelmin.text())) / (
                    int(self.levelnum.text()) - 1
                )
    ...
    def set_var_props(self, var):
        if self.ready:
            ...
            self.contcolor.setStyleSheet(
                "background-color: {}".format(self.props[var]["color"])
            )
                    ...
                    if self._model.item(i).checkState():
                        ...
                        if self.props[var]["min"] == self.props[var]["max"]:
                            ...
                            if self.props[var]["type"] == "linear":
                                ...
                                data = np.ma.masked_outside(
                                    data, self.props[var]["min"], self.props[var]["max"]
                                )
                                v = np.sort(data.compressed())
                                v = v[np.invert(np.isnan(v))]
                                IQR = (
                                    v[int(round((v.size - 1) * float(0.75)))]
                                    - v[int(round((v.size - 1) * float(0.25)))]
                                )
                                ...
                                cdf = np.cumsum(hist)
                                cdfx = (
                                    np.cumsum(np.diff(bin_edges))
                                    + bin_edges[:2].sum() / 2
                                )
                                # clevels = np.interp(np.linspace(cdf[0],cdf[-1],self.props[var]['num'] + 2)[1:-1], cdf, cdfx)
                                clevels = np.interp(
                                    np.linspace(
                                        cdf[0], cdf[-1], self.props[var]["num"]
                                    ),
                                    cdf,
                                    cdfx,
                                )
                        clevels = (
                            np.round(10 ** self.props[var]["digits"] * clevels)
                            / 10 ** self.props[var]["digits"]
                        )
                        # Contour levels must be increasing
                        clevels = np.append(
                            clevels[:1], clevels[1:][np.diff(clevels) > 0]
                        )
                        if self.props[var]["contours"] == "map":
                            CS = self._ax.contour(
                                self.data.get_xrange(self.props[var]["resample"]),
    ...
    def check(self):
        try:
            ...
            QtWidgets.QMessageBox.warning(
                self, "Warning", "Not a number must be float number or NaN"
            )
...
class AboutDialog(QtWidgets.QDialog):
    ...
    def __init__(self, version, parent=None):
        ...
        about = QtWidgets.QLabel(
            "PyWerami {}\nstand-alone program to make an countour/3D plot from a contour data".format(
                version
            )
        )
...
def process_cl_args():
    ...
    parser.add_argument(
        "filename", action="store", nargs="?", default=None, help="Data file"
    )
