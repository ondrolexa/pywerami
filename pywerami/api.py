...
class GridData:
    ...
    def from_tci(cls, filename, degrees=True):
        ...
        if degrees:
            ...
            vx = dict(
                name="T(K)", min=x.min() + 273.15, num=len(x), max=x.max() + 273.15
            )
    ...
    def get_xrange(self, divider=1):
        return np.linspace(
            self.ind[self.xvar]["min"],
            self.ind[self.xvar]["max"],
            divider * self.ind[self.xvar]["num"],
        )

    def get_yrange(self, divider=1):
        return np.linspace(
            self.ind[self.yvar]["min"],
            self.ind[self.yvar]["max"],
            divider * self.ind[self.yvar]["num"],
        )
